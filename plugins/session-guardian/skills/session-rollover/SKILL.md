...
6. Navigate to the ready replacement with `navigate_to_codex_page`, passing its actual `threadId`. Do not navigate to the generic new-conversation page or pass a setup-only `clientThreadId`.
7. If the current task is known to be pinned, leave it unarchived and tell the user. If `archive_original` is disabled, also leave it unarchived.
8. Otherwise, tell the user the replacement is ready and that this task will now be archived, then archive the calling task with the Codex app archival tool. Target the calling task by omitting a task id; never guess an id.
