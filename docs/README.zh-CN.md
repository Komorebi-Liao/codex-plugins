...
1. 64 MiB：提示会话已经偏大；
2. 96 MiB：在回合完成后自动交接，同时要求当前任务至少提交过 6 次提示，以降低单个大附件造成的误报；
3. 128 MiB：硬保护，不再执行新的用户请求，立即启动交接，不受提示次数限制。

会话日志大小是唯一的自动检测信号。插件不读取 Clash/Mihomo 连接，不采集代理流量、系统网络计数器或数据包；未安装任何代理软件也不影响使用。

达到 96 MiB 条件后，插件会等待当前回合结束；达到 128 MiB 时，则会拦截新的用户提示并立即开始以下流程：

1. 通过本地 Codex App Server 让旧任务生成结构化交接摘要；这是旧任务最后一次携带完整上下文的请求，用于保证交接质量；
